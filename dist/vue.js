(function (global, factory) {
  ...
  function patch(oldVnode, vnode) {
    ...
    if (!oldVnode) {
      ...
      if (isRealElement) {
        ...
        var _el = vnode.el = oldVnode.el; // 为了节点复用 所以直接把旧的虚拟dom对应的真实dom赋值给新的虚拟dom的el属性
        ...
        if (oldCh.length > 0 && newCh.length > 0) {
          // 3.1. 新老都存在子节点
          updateChildren(_el, oldCh, newCh); // 【核心算法】
  ...
  function updateChildren(parent, oldCh, newCh) {
    ...
    var oldEndVnode = oldCh[oldEndIndex]; //老儿子的结束节点

    var newStartIndex = 0; // 新儿子的，同上

    var newStartVnode = newCh[0];
    var newEndIndex = newCh.length - 1;
    var newEndVnode = newCh[newEndIndex]; // 根据key来创建老的儿子的index映射表；类似 {'a':0,'b':1}：表示key为'a'的节点在第一个位置，key为'b'的节点在第二个位置
    ...
    var keysMap = makeIndexByKey(oldCh); // 只有当新老儿子的双指标的起始位置不大于结束位置的时候，才能循环；
    ...
    while (oldStartIndex <= oldEndIndex && newStartIndex <= newEndIndex) {
      ...
      if (!oldStartVnode) {
        ...
        // 头尾比较
        patch(oldStartVnode, newEndVnode); //  比较完，就需要将递归的结果，放到oldEndVnode后面（因为新的是在尾部，所以当头尾比较满足samavnode时，需要将老的vnode移到尾部，与newCh顺序保持一致）
        ...
        // 尾头比较
        patch(oldEndVnode, newStartVnode); //  比较完，就需要将递归的结果，放到oldStartVnode前面（因为新的是在头部，所以当尾头比较满足samavnode时，需要将老的vnode移到头部，与newCh顺序保持一致）

        parent.insertBefore(oldEndVnode.el, oldStartVnode.el); // 指针改变，oldEndVnode、newStartVnode相应改变
    ...
    if (newStartIndex <= newEndIndex) {
      ...
      for (var i = newStartIndex; i <= newEndIndex; i++) {
        ...
        var anchor = newCh[newEndIndex + 1] == null ? null : newCh[newEndIndex + 1].el;
  ...
  function lifecycleMixin(Vue) {
    ...
    Vue.prototype._update = function (vnode) {
      var vm = this;
      var prevVnode = vm._vnode; // 获取上一次的vnode
  ...
  function createComponent(vm, tag, data, key, children, Ctor) {
    ...
    return new Vnode("vue-component-".concat(tag), data, key, undefined, undefined, {
      Ctor: Ctor,
      children: children
    });
  } // 创建文本vnode
